				...
				const gchar *path, const gchar *sender)
{
	...
	_bt_ag_agent_emit_signal(bt_ag_info.conn, path,
...
static gboolean __bt_ag_codec_negotiation_finished(gpointer data)
{
	if (g_strcmp0((char *)data, "finish") == 0) {
		DBG("Codec negotiation finished");
		__bt_ag_sco_connect(&bt_ag_info);
		__bt_ag_codec_negotiation_info_reset(FALSE);
		return TRUE;
	} else if (g_strcmp0((char *)data, "timeout") == 0) {
...
bt_hfp_agent_error_t _bt_ag_set_codec(char *method)
{
	GDBusProxy *proxy;
	GVariant *ret;
	GError *err = NULL;
...
static int __bt_ag_start_sco_server(bt_ag_info_t *hs)
{
	...
	_bt_convert_addr_string_to_type_rev(bd_addr.b, bt_ag_info.remote_addr);
	DBG("Bind to address %s", bt_ag_info.remote_addr);
...
static gboolean __bt_sco_open_delay_timeout_cb(gpointer user_data)
{
	...
	if (sco_open_request && bt_ag_info.state == HEADSET_STATE_CONNECTED) {
		...
		if ((ag.features & BT_AG_FEATURE_CODEC_NEGOTIATION) &&
				(slconn && (slconn->hs_features &
					BT_HF_FEATURE_CODEC_NEGOTIATION))) {
			switch (codec_info.final_codec) {
			case BT_CVSD_CODEC_ID:
				_bt_ag_set_codec("SetNbParameters");
				__bt_ag_sco_connect(&bt_ag_info);
				break;
			case BT_MSBC_CODEC_ID:
				_bt_ag_set_codec("SetWbsParameters");
				__bt_ag_sco_connect(&bt_ag_info);
				break;
			default:
				__bt_hfp_codec_connection_setup(FALSE);
...
void _bt_ag_slconn_complete(bt_ag_info_t *hs)
{
	...
	sco_open_timer_id = g_timeout_add(BT_SCO_OPEN_DELAY_TIMER,
						__bt_sco_open_delay_timeout_cb, NULL);
			...
			gpointer user_data)
{
	...
	if (g_strcmp0(method_name, "NewConnection") == 0) {
		...
		__bt_convert_addr_type_to_rev_string(hdset_address,
				(unsigned char *)bt_ag_info.remote_addr);
		...
		name_owner_sig_id = g_dbus_connection_signal_subscribe(bt_ag_info.conn,
...
static void __bt_ag_agent_sigterm_handler(int signo)
{
	...
	if (bt_ag_info.state >= HEADSET_STATE_CONNECTED) {
		...
		if (bt_ag_info.conn)
			g_dbus_connection_flush(bt_ag_info.conn, NULL, NULL, NULL);
	}
...
#else	/* Kiran only TODO :: need to chagne properly */
